...
export default function MotDePasseOublie() {
  ...
  const etape1 = async (e) => {
    ...
    try {
      ...
      toast.success('Code généré !'); setEtape(2)
  ...
  const etape2 = async (e) => {
    ...
    try {
      ...
      setSuccess(true); toast.success('Mot de passe réinitialisé !')
    ...
    <div className="auth-page">
      <div className="auth-card">
        <div className="auth-logo">
          ...
          <p>Portail Personnel Universitaire</p>
        </div>
        <div style={{ textAlign:'center' }}><span className="auth-badge">Espace Personnel</span></div>
        ...
        {success ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <Link to="/connexion" className="btn btn-primary btn-full" style={{ textDecoration:'none' }}>
              Se connecter
            </Link>
          </div>
        ) : (
          <>
            ...
            {etape === 1 ? (
              <form onSubmit={etape1} noValidate>
                <p style={{ color:'var(--gray-500)', fontSize:'0.875rem', marginBottom:20 }}>
                  Entrez votre email universitaire pour recevoir un code de réinitialisation.
                </p>
                ...
                <button type="submit" className="btn btn-primary btn-full" disabled={loading}>
                  {loading ? <div className="spinner" /> : null}{loading ? 'Envoi...' : 'Envoyer le code'}
                </button>
              </form>
            ) : (
              <form onSubmit={etape2} noValidate>
                {codeDev && (
                  <div style={{ background:'#fef9c3', border:'1px solid #fde047', borderRadius:8, padding:'10px 14px', fontSize:'0.82rem', color:'#854d0e', marginBottom:16 }}>
                    <strong>🔑 Code (mode dev) :</strong> {codeDev}
                  </div>
                )}
                <div className="form-group">
                ...
                <button type="submit" className="btn btn-primary btn-full" disabled={loading}>
                  {loading ? <div className="spinner" /> : null}{loading ? 'Réinitialisation...' : 'Réinitialiser'}
                </button>
