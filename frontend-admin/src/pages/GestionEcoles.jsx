...
import { Plus, Pencil, Trash2, School, RefreshCw, Check, X, Search } from 'lucide-react'
...
function FormEcole({ initial, onSave, onCancel }) {
  const [form, setForm] = useState(initial
    ? { nom: initial.nom || '', sigle: initial.sigle || '', ville: initial.ville || '', pays: initial.pays || 'Sénégal', description: initial.description || '' }
    : { ...EMPTY })
      ...
      <div className="form-row" style={{ marginBottom: 12 }}>
        <div className="form-group" style={{ marginBottom: 0 }}>
          ...
          <div className="input-wrapper">
            <School size={15} className="input-icon" />
            <input name="nom" value={form.nom} onChange={set} placeholder="École Supérieure Polytechnique" />
          </div>
        </div>
        <div className="form-group" style={{ marginBottom: 0 }}>
          <label>Sigle</label>
          <div className="input-wrapper">
            <input name="sigle" value={form.sigle} onChange={set} placeholder="ESP" style={{ paddingLeft: 14 }} />
          </div>
        </div>
      </div>
      <div className="form-row" style={{ marginBottom: 12 }}>
        <div className="form-group" style={{ marginBottom: 0 }}>
          <label>Ville</label>
          <div className="input-wrapper">
            <input name="ville" value={form.ville} onChange={set} placeholder="Dakar" style={{ paddingLeft: 14 }} />
          </div>
        </div>
        <div className="form-group" style={{ marginBottom: 0 }}>
          <label>Pays</label>
          <div className="input-wrapper">
            <input name="pays" value={form.pays} onChange={set} placeholder="Sénégal" style={{ paddingLeft: 14 }} />
          </div>
        </div>
      </div>
      <div className="form-group" style={{ marginBottom: 12 }}>
        ...
        <div className="input-wrapper">
          <textarea name="description" value={form.description} onChange={set} rows={2}
            placeholder="Description optionnelle..." style={{ paddingLeft: 14, resize: 'vertical' }} />
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <button type="button" className="btn btn-ghost btn-sm" onClick={onCancel}><X size={14} />Annuler</button>
        <button type="submit" className="btn btn-primary btn-sm">
          <Check size={14} />{initial ? 'Enregistrer' : 'Ajouter'}
        </button>
      </div>
    </form>
  )
...
export default function GestionEcoles() {
  ...
  const fetchEcoles = () => {
    ...
    api.get('/ecoles/admin/liste')
      .then(r => { setEcoles(r.data.data || []); setFiltered(r.data.data || []) })
  ...
  useEffect(() => {
    if (!search.trim()) { setFiltered(ecoles); return }
    const q = search.toLowerCase()
    setFiltered(ecoles.filter(e =>
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          <p>Gérer les écoles disponibles à l'inscription</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-ghost btn-sm" onClick={fetchEcoles}><RefreshCw size={14} />Actualiser</button>
          {!showForm && !editEcole && (
      ...
      <div className="table-container">
        ...
        <div className="table-header">
          <div className="search-box">
            ...
            <input
              type="text"
              placeholder="Rechercher par nom, sigle ou ville..."
              value={search}
              onChange={e => setSearch(e.target.value)}
            />
          </div>
          <span style={{ fontSize: '0.82rem', color: 'var(--gray-400)' }}>
            {filtered.length}/{ecoles.length} établissement(s)
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            <School size={40} />
            <p>{search ? 'Aucun résultat pour cette recherche.' : 'Aucun établissement enregistré.'}</p>
            {!search && <p style={{ marginTop: 6, fontSize: '0.8rem' }}>Cliquez sur "Nouvel établissement" pour commencer.</p>}
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map(ecole => (
                  <tr key={ecole.id}>
                    ...
                    <td>
                      {ecole.sigle && (
                        <span className="badge" style={{ background: '#eff6ff', color: '#1d4ed8' }}>{ecole.sigle}</span>
                      )}
                    </td>
                    ...
                    <td>
                      <div className="actions-cell">
                        <button className="btn btn-ghost btn-sm" title="Modifier"
                          onClick={() => { setShowForm(false); setEditEcole(ecole) }}>
                          <Pencil size={14} />
                        </button>
                        <button
